 ...
 * https://github.com/rowlandm/biojs-vis-violin-plot
 *
...
var  biojsvisviolinplot;

module.exports = biojsvisviolinplot = function(init_options)
{
    ...
    */
    ...
    this.setup_x_axis = function (graph){
        ...
        var scaleX = d3.scale.ordinal()
            .rangeBands([page_options.initial_padding, page_options.width]); // note that 0.4 was chosen by iterative fiddling
    ...
    this.setup_error_bars = function (graph){
        ...
        */
        ...
        svg.selectAll(".max").data(options.data).enter()
            ...
            .attr("x1", 
                function(d){
                        ...
                        if(((d.Expression_Value) - d.Expression_Value) < (d.Expression_Value/dividor)){
            ...
            .attr("y1", 
                function(d){
                    if(((d.Expression_Value) - d.Expression_Value) > 0){
                        temp = scaleY(d.Expression_Value);//upper value
                        return temp;
                    }else{
                        return 0;
                    }
        ...
        svg.selectAll(".min").data(options.data).enter()
            ...
            .attr("y1", 
                function(d){
                    temp = scaleY(d.Expression_Value);//lower value
                    return temp;
                }
            )
            .attr("y2", 
        ...
        svg.selectAll(".vertical").data(options.data).enter()
            ...
            .attr("y1", 
                function(d){
                    temp = scaleY(d.Expression_Value);//
                    return temp;
                }
            )
            .attr("y2", 
    ...
    this.setup_violin = function(graph){
        ...
        */
        ...
        // example thanks to http://bl.ocks.org/z-m-k/5014368
        ...
        imposeMax = 1;
        var violinColor = "#5a5a5a";
        var width=graph.options.width;
        var height=graph.options.height;
        var boxWidth=20;
        var boxSpacing=10;
        ...
        for (temp_count in nested_values){
            count = count + 1;
            var new_g=svg.append("g").attr('class','nothing').attr("transform", "translate("+(count*(boxWidth+boxSpacing)+left)+",0)");
            row = nested_values[temp_count];
            key = row.key;
            values = row.values; 
            results = Array();
            for (count_row in values){
                ...
                sample_id = sample.Sample_ID;
                sample_type = sample.Sample_Type;
                expression_value = sample.Expression_Value;

                results.push(expression_value);
            ...
            var gPlus=new_g.append("g").attr('class','gPlus');
            var gMinus=new_g.append("g").attr('class','gMinus');
        ...
        svg.selectAll(".bar")
            ...
            .attr("x", function(d){
                ...
                if(first == 0) {
                    ...
                    return scaleX(d.start_sample_id);
            ...
            .attr("width", function(d, i) { 
                if(once == 0) {
                    once = 1;
                    return scaleX(d.end_sample_id) - scaleX(d.start_sample_id);
                } if (scaleX(d.end_sample_id) >= options.width - (2 * padding)) {
                    //if it is the last sample type need to account for padding of the graph
                    //append 15 to the bar
