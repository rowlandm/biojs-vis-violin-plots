    ...
    */
    ...
    this.setup_violin = function(graph){
        ...
        */
        ...
        for (temp_count in nested_values){
            count = count + 1;
            var g=svg.append("g").attr("transform", "translate("+(count*(boxWidth+boxSpacing)+left)+",0)");
            row = nested_values[temp_count];
            key = row.key;
            values = row.values; 
            results = Array();
            ...
            gPlus.attr("transform", "rotate(90,0,0)  translate(0,-1)");//translate(0,-200)");
